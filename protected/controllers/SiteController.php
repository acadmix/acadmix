...
class SiteController extends RController
{
	...
	public function actionLogin()
	{
		...
		if(isset($_POST['LoginForm']))
		{
			...
			if($model->validate() && $model->login() && $model->isActivated())
			{
                            session_start();
                            $_SESSION['userforsetup'] = "true";
				$this->redirect(Yii::app()->user->returnUrl);
			}
                        elseif($model->validate() && $model->login() && !$model->isActivated())
                        {
                            $this->redirect(array('user/changePass&id='.Yii::app()->user->id));
                        }
		}
	...
	{
            $_SESSION['userforsetup'] = "false";
            setcookie("payroll_school_id", "", time() - 3600);
                Yii::app()->user->logout();
