...
class DefaultController extends CController
{
    ...
    */
    public function actionIndex() {
        ...
        if(Yii::app()->session['school_id']!="osv_ms" || !isset($_SESSION['userforsetup']) || $_SESSION['userforsetup']==false)
        {
            
            $this->redirect(array('error403'));
        }
				...
				if($model->checkConnection() !== true) {
					//Attemting To Create The Database
					 ...
					 if (mysql_query("CREATE DATABASE $model->dbName"))
                                         {
                                            $flag_db_created = 1;
                                            Yii::app()->session['school_id'] = $model->dbName;
                                         }
					 else{
					 $flag_db_created = 0;
					 }
...
//                            Yii::trace('$script:'.$script);
                ...
                $user->school_id = Yii::app()->session['school_id'];
                $user->username = 'admin_'.Yii::app()->session['school_id'];
                $user->type = '1';
               ...
               // $user->FirstName = 'Site';
                ...
                Yii::app()->db->connectionString = DB_DEFAULT_EMPTY.Yii::app()->session['school_id'];
                Yii::app()->db->setActive(true);
                $admin_user = User::model()->findByPk(1);
                $admin_user->setScenario('init');
                $admin_user->school_id = Yii::app()->session['school_id'];
                $admin_user->username = 'admin_'.Yii::app()->session['school_id'];
                $admin_user->type = '1';
                $admin_user->salt = $salt;
                $admin_user->password = $user->password;
                
                if ($admin_user->save() === true) {
                    Yii::app()->session['aemail'] = 'admin_'.Yii::app()->session['school_id'];
